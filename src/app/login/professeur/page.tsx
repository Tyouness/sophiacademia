...
export const metadata: Metadata = {
  title: "Connexion Professeur | Sophiacademia",
  description: "Accédez à votre espace professeur Sophiacademia.",
};

type Props = { searchParams?: Promise<{ error?: string; sent?: string }> };

export default async function LoginProfesseurPage({ searchParams }: Props) {
  ...
  return (
    <main>
      {/* fond légèrement différent pour distinguer visuellement de la page famille */}
      <section
        style={{ background: "linear-gradient(135deg, #0F172A 0%, #0d2640 60%, #1E3A5F 100%)" }}
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8 flex items-center gap-3">
            ...
            <span
              ...
              style={{ backgroundColor: "rgba(30,58,95,0.6)", color: "#93C5FD", border: "1px solid rgba(147,197,253,0.2)" }}
            >
              Espace Professeur
            </span>
          </div>
          ...
          >
            <p className="mb-1 text-xs font-semibold uppercase tracking-widest" style={{ color: "#93C5FD" }}>
              Connexion
            </p>
            <h1
            ...
            <div className="mt-2 mb-6 h-[2px] w-8 rounded-full" style={{ backgroundColor: "#93C5FD" }} aria-hidden />
            <p className="mb-8 text-sm leading-6" style={{ color: "#94A3B8" }}>
              Gérez vos cours, consultez vos fiches de paie et répondez aux demandes des familles.
            </p>

            <form method="POST" action="/auth/login" className="space-y-4">
              {/* role_hint — champ caché pour vérification côté serveur */}
              <input type="hidden" name="role_hint" value="professeur" />
              ...
              <button
                ...
                style={{ backgroundColor: "#93C5FD", color: "#0F172A" }}
              >
                Se connecter
              </button>
            </form>
          ...
          {/* Switch vers login famille */}
          <p className="mt-6 text-center text-xs" style={{ color: "#475569" }}>
            Vous êtes une famille ?{" "}
            <Link href="/login/famille" className="font-semibold hover:text-amber-400 transition-colors" style={{ color: "#F59E0B" }}>
              Espace Famille →
            </Link>
          </p>
        </div>
