...
export default async function StaffRequestsPage() {
  ...
  const { data: requests } = await supabase
    .from("requests")
    .select(
      "id, professor_id, family_id, subject, status, created_at, professor:profiles!requests_professor_id_fkey(full_name, email, phone, addr1, addr2, postcode, city, country, lat, lng), family:profiles!requests_family_id_fkey(full_name, email, phone, addr1, addr2, postcode, city, country, lat, lng)",
    )
    ...
    <main className="space-y-6">
      <div className="rounded-xl bg-white p-6 shadow-md">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Valider ou refuser les demandes professeurs.
        </p>
      </div>

      <RequestListTable requests={rows} />
    </main>
  );
}
